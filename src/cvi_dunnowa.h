...
int DuNNOWA_get_OWA(std::string owa_name)
{
    if      (owa_name == "Mean")   return OWA_MEAN;
        ...
        CVI_ASSERT(delta > 0 && delta < OWA_SMIN_LIMIT-OWA_SMIN_START);
        return OWA_SMIN_START+delta;
    }
    else if (owa_name.substr(0, 5) == "SMax:") {
        int delta = std::atoi(owa_name.substr(5).c_str());
        CVI_ASSERT(delta > 0 && delta < OWA_SMAX_LIMIT-OWA_SMAX_START);
 ...
 *
 *  TODO: Proposed by Gagolewski
 *  TODO: Inspired by generalised Dunn indexes <CITE>
 *
