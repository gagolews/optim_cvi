...
public:
    ...
    const FLOAT_T operator()(size_t i, size_t j) const
    {
        ...
        if (precomputed) {
            if (i > j) std::swap(i, j);
            //CVI_ASSERT(i*n - i*(i+1)/2+(j-i-1) >= 0);
            //CVI_ASSERT(i*n - i*(i+1)/2+(j-i-1) < D.size());
...
public:
    ...
    {
        CVI_ASSERT(X.nrow() == _L.size());
        for (size_t j=0; j<K; ++j) {
            count[j] = 0;
        }

        for (size_t i=0; i<n; ++i) {
            CVI_ASSERT(_L[i] >= 0 && _L[i] < K);
            L[i] = _L[i];
            count[ L[i] ]++;
        }

        for (size_t j=0; j<K; ++j) {
            CVI_ASSERT(count[j] > 0);
        }
    }
    ...
    virtual void modify(size_t i, uint8_t j)
    {
        CVI_ASSERT(i >= 0 && i < n);
        CVI_ASSERT(j >= 0 && j < K);
        CVI_ASSERT(L[i] >= 0 && L[i] < K);
        CVI_ASSERT(count[L[i]] > 0);
        CVI_ASSERT(L[i] != j);
