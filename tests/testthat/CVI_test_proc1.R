...
CVI_test_proc1 <- function(CVI_name, CVI_fun, reference_fun)
{
    ...
    test_that(CVI_name, {
        ...
        for (u in seq_along(Xs)) {
            ...
            i3 <- {.CVI_set_labels(cvi_ptr, y); .CVI_compute(cvi_ptr)}
            expect_equivalent(i3, i1)

            if (!is.null(reference_fun)) {
                print(microbenchmark::microbenchmark(
                    direct={i1 <- CVI_fun(X, y, K)},
                    precomputed={i3 <- {.CVI_set_labels(cvi_ptr, y); .CVI_compute(cvi_ptr)}},
            ...
            for (v in 1:50) {
                i <- sample(n, 1)
                j <- sample(K, 1)
                if (y[i] == j || tabulate(y)[y[i]] <= 1) next
                y[i] <- j
